...
- `manifest.jsonl` — [机器真相层] 全局单行 JSON 索引，供 Agent 秒级结构化检索。
...
- **L1 优选（极速收敛，5ms / 200 Token）**：
  ...
  ```
- **L2 降级（人类看板兜底）**：
  若当前环境缺失 `jq` 或 Python 运行受阻，降级阅读编译生成的人类看板：
  - 查阅文档分类：阅读 `.context/docs/INDEX.md`；
  - 查阅任务进展与停滞预警：阅读 `.context/tasks/STATUS.md`。
- **L3 极限兜底（文件系统直查）**：
  若看板尚未生成或损坏，直接使用目录查找：
...
- **Frontmatter 强制要求**：新建或编辑文档时，头部必须包含合法的 YAML Frontmatter（`type`、`title`、`status`、`summary` 为必填项）。
- **严格英文状态枚举**：统一严格使用：`active` | `draft` | `in_progress` | `paused` | `completed` | `resolved` | `archived`。
- **只读派生层禁止手改**：`docs/INDEX.md`、`tasks/STATUS.md`、`CLEANUP.md` 与 `manifest.jsonl` 由同步脚本自动编译，**严禁手动编辑**。
- **修改时效维护**：编辑文档后，将 `updated:` 更新为当天（`YYYY-MM-DD`）；若配置了 Hook 守护，该动作由 Hook 自动且幂等完成。

### 2.3 异常自愈与降级机制 (Graceful Fallback & Self-Healing)
- **索引损坏一键重构**：若 `manifest.jsonl` 发生冲突或格式损坏，运行 `python3 .context/scripts/sync_bundle.py` 即可在毫秒内根据真相层从头重建全部索引与看板。
